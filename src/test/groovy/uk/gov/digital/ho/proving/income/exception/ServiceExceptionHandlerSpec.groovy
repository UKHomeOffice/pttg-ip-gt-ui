package uk.gov.digital.ho.proving.income.exception
...
class ServiceExceptionHandlerSpec extends Specification {
    ...
    def 'converts RestServiceExceptions emerging from API'() {
        ...
        HttpStatus.INTERNAL_SERVER_ERROR | HttpStatus.INTERNAL_SERVER_ERROR | 'Error at Income Proving API server'
        HttpStatus.BAD_GATEWAY           | HttpStatus.INTERNAL_SERVER_ERROR | 'API response status: 502'
        HttpStatus.BAD_REQUEST           | HttpStatus.INTERNAL_SERVER_ERROR | 'Bad request to Income Proving API server'
    }
