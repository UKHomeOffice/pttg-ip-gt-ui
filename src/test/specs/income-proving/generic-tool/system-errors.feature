Feature: System errors - specify messages shown in response to (simulated) connection failures etc
    ...
    If the API responds with a valid HTTP response, whether success, client error, Page dynamic heading,'t retry.
    Note that we have to pause the scenario before verifying the number of calls, to allow for any retr y attempts (that shouldn't not happen)
        Given the api response has status 503
        When the income check is performed
